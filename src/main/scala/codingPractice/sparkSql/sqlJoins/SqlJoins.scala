package codingPractice.sparkSql.sqlJoins
...
object SqlJoins extends App{
  ...
  val spark = SparkSession
    .builder()
    .config("spark.sql.warehouse.dir", "src/main/scala/codingPractice/sparkSql/warehouse")
    .master("local[1]")
    .appName("SqlJoins")
    .getOrCreate()
  ...
  val salesmanInput = spark
    .sparkContext
    .textFile("src/main/scala/codingPractice/sparkSql/sqlJoins/data/salesman.txt")
    .map(line => line.split("\\|"))
  ...
  val customerInput = spark
    .sparkContext
    .textFile("src/main/scala/codingPractice/sparkSql/sqlJoins/data/customer")
    .map(line => line.split("\\|"))
  ...
  val ordersInput = spark.sparkContext.textFile("src/main/scala/codingPractice/sparkSql/sqlJoins/data/orders")
    .map(line => line.split(","))
    .map(
      rec =>
        Orders(
  ...
  val customer1Df = spark
    ...
    .option("sep", ",")
    .csv("src/main/scala/codingPractice/sparkSql/sqlJoins/data/customer1.csv")
